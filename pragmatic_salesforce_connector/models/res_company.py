...
class ResCompany(models.Model):
    ...
    @api.model
    def _default_start_update_datetime(self):
        date = str(datetime.strptime("2019-12-01 00:00:00", "%Y-%m-%d %H:%M:%S"))
        return datetime.strptime("2019-12-01 00:00:00", "%Y-%m-%d %H:%M:%S")
    ...
    contact_startmodifieddate = fields.Datetime("Contact Start Modified Time :- ")
    ...
    opportunity_startmodifieddate = fields.Datetime("Opportunity Start Modified Time :- ", default=_default_start_update_datetime)
    ...
    event_startmodifieddate = fields.Datetime("Event Start Modified Time :- ")
    task_lastmodifieddate = fields.Datetime("Task Last Modified Time :- ", default=_default_update_datetime)
    task_startmodifieddate = fields.Datetime("Task Start Modified Time :- ")
    ...
    def import_sf_activity(self):
        try:
            if self.sf_access_token:
                ...
                start_temp_odoo_date = self.task_startmodifieddate
                sf_modified_date = self.convert_odoodt_tosf(str(temp_odoo_date))
                sf_start_modified_date = self.convert_odoodt_tosf(str(start_temp_odoo_date))
                data = requests.request('GET', self.sf_url + "/services/data/v40.0/query/?q=select Id from Task where (LastModifiedDate < {} AND LastModifiedDate > {}) ORDER BY LastModifiedDate".format(sf_modified_date,sf_start_modified_date), headers=headers)
                if data:
                    recs = []
                    parsed_data = json.loads(str(data.text))
                    print("\n\n==== data ----",parsed_data)
                    ...
                    for rec in recs:
                        ...
                        if not activity_read.get('WhoId') and activity_read.get('WhatId'):
                            ...
                            # activity_dict['res_id'] = res_partner.id
                                ...
                                _logger.info("Who/what >>>>>>>>>>>>>>>>>>> : ")
                                # raise UserError(_(
                                #     "WhatId Not found in the record please add it first for id " + activity_read.get(
                                #         'Id')))
                        else:
                            _logger.info("Who/what 2>>>>>>>>>>>>>>>>>>> : ")
                            # raise UserError(_("WhatId Not found in the record please add it first for id " + activity_read.get('Id')))
